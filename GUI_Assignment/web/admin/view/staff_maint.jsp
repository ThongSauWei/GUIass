...
<%
    ...
    if (delete != null && id != null) {
        if (new StaffController().dltStaff(Integer.parseInt(id))) {
            response.sendRedirect("staff_list.jsp?delete=1");
            return;
        } else {
    ...
    if (request.getParameter("submit") != null) {
        ...
        if (submit == 1) {
            ...
            if (new StaffController().addStaff(name, password, ic, phoneNum, email, birthday)) {
                ...
                if (action == 2) {
                    response.sendRedirect("staff_maint.jsp?isNew=true&action=" + action + "");
                    return;
                }
                if (action == 3) {
            ...
            String name = request.getParameter("name");
            String desc = request.getParameter("phoneNum");
            String phoneNum = request.getParameter("phoneNum");
            String email = request.getParameter("email");
            String birthday = request.getParameter("birstday");
            String ic = request.getParameter("ic");
            String password = request.getParameter("password");
            action = Integer.parseInt(request.getParameter("action"));

            if (new StaffController().updateStaff(name, desc, phoneNum, email, birthday, ic, password)) {
                if (action == 1) {
                    response.sendRedirect("staff_list.jsp");
                    return;
                }
                ...
                <table class="table table-striped table-dark">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span id="password_error" class="error-message"></span><input onblur="passwordValid()" type="text" id="password" name="password" class="error-border form-control" value="<%=isNew ? "" : product.getStaffPass()%>"></td>
                        </tr>
                        <tr>
                            <th scope="row">Confirm Password</th>
                            <td><span id="cf_password_error" class="error-message"></span><input onkeyup="cf_password_onkeyup()" type="text" id="cf_password" name="cf_password" class="error-border form-control" value="<%=isNew ? "" : product.getStaffPass()%>"></td>
                        </tr>
                    </tbody>
                </table>
